...
export default {
  ...
  activated(){
    ...
    setTimeout(()=>{ // 设置 延时50毫秒 绑定事件（因为在路由切换过程中会触发 scroll 事件）
      this.scrollParent = This.isWindowScroll();// 确定 监听 scroll 事件的 dom节点
      this.scrollParent.addEventListener('scroll', This.scrollFn, false) // 添加事件监听
    },50)
  },
  deactivated(){  
    //let scrollParent = this.isWindowScroll() 
    this.scrollParent.removeEventListener('scroll',this.scrollFn, false) // 解除事件监听*/
  }
}
</script>
